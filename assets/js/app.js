...
// Tipos especiais:
//   { type:'mes',       label:'Extintor' }        → seletor MÊS/ANO
...
// Atribuição de carreta para NOK do SR quando rodotrem ativo
// n → 'CARRETA 1' | 'CARRETA 2' | 'AMBAS'
const nokCarreta = {};
...
(function renderizar() {
  for (const [bid, grp] of Object.entries(MAP)) {
    ...
    ITENS[grp].forEach(([n, d]) => {
      ...
      // listeners via JS (necessário para callback do popup funcionar)
...
/* ══════════════════════════════════════════════════════
   POPUP CARRETA (callback-based, sem async/await)
   ══════════════════════════════════════════════════════ */
let _cbCarreta = null;
...
/* ══════════════════════════════════════════════════════
   ...
   ══════════════════════════════════════════════════════ */
const SEP = '\u200B'; // separador invisível entre texto manual e automático
...
/* ══════════════════════════════════════════════════════
   CANVAS DE ASSINATURA (alta resolução)
   ══════════════════════════════════════════════════════ */
...
function sincEtapa() {
  ...
  [1, 2].forEach(i => {
    ...
    bar.classList.toggle('feito', i < etapa);
  });
  if (etapa === 3) {
...
function gerarPDF() {
  ...
  function cabecalho() {
    ...
    [gv('data'), gv('revisao') || '00', '1 de 2', gv('departamento') || 'Frota / Manutencao']
      .forEach((v, i) => { doc.text(v, x + 1, y + 4.8); x += c1[i]; });
    y += 6.5;
  ...
  function iRow(n, d, par) {
    ...
    doc.text(
      `(${ck.includes('OK') ?'X':' '}) OK  (${ck.includes('NOK')?'X':' '}) NOK  (${ck.includes('NA') ?'X':' '}) NA`,
      ML + cW - 49, y + 2.7
    );
    y += rh;
  }
  ...
  function observacoes() {
    ...
    for (let i = 1; i * 4 < oH - 2; i++) {
      doc.setDrawColor(220); doc.line(ML + 2, oTop + i * 4, ML + cW - 2, oTop + i * 4);
    }
...
function gerarCSV(placa, data) {
  ...
  const dataFmt = gv('data')
    ? gv('data').split('-').reverse().join('/')
    : data;
  ...
  const cabecalho = '"Data","Placa Cavalo","Placa SR1","Placa SR2","Tipo","KM","Motorista","Conferente","Observações"';
  const csvContent = '\uFEFF' + cabecalho + '\n' + linha; // BOM para Excel reconhecer UTF-8

  const csvBlob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
  const csvUrl  = URL.createObjectURL(csvBlob);
  const link    = document.createElement('a');
  link.href     = csvUrl;
  link.download = `Checklist_${placa}_${data}.csv`;
